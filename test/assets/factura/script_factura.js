...
function infocliente(id) {
    ...
    $.ajax({
        ...
        success: function (html) {
            console.log(html);
            if (html.top1 !== " | ") { $(".top1").html(html.top1); }  else { $(".top1").html("Cliente Nuevo"); }
            if (html.top1cant !== null) { $(".top1cant").html(html.top1cant); }  else { $(".top1cant").html("0"); }
            if (html.comprado !== null) { $(".comprado").html("$ " + html.comprado); } else { $(".comprado").html("$ 0.00"); }
            if (html.last1 !== null) { $("#last1 .widget-title").html("FACTURA # " + html.last1); } else { $("#last1 .widget-title").html("Cliente Nuevo"); }
            if (html.last1date !== null) { $("#last1 .widget-subtitle").html(html.last1date); } else { $("#last1 .widget-subtitle").html("No hay ventas"); }
            if (html.last1tot !== null) { $("#last1 .widget-int").html(html.last1tot); } else { $("#last1 .widget-int").html("0.00"); }
            if (html.last2 !== null) { $("#last2 .widget-title").html("FACTURA # " + html.last2); } else { $("#last2 .widget-title").html("Sin Datos"); }
            if (html.last2date !== null) { $("#last2 .widget-subtitle").html(html.last2date); } else { $("#last2 .widget-subtitle").html("No hay mas ventas"); }
            if (html.last2tot !== null) { $("#last2 .widget-int").html(html.last2tot); } else { $("#last2 .widget-int").html("0.00"); }
            if (html.last3 !== null) { $("#last3 .widget-title").html("FACTURA # " + html.last3); } else { $("#last3 .widget-title").html("Sin Datos"); }
            if (html.last3date !== null) { $("#last3 .widget-subtitle").html(html.last3date); } else { $("#last3 .widget-subtitle").html("No hay mas ventas"); }
            if (html.last3tot !== null) { $("#last3 .widget-int").html(html.last3tot); } else { $("#last3 .widget-int").html("0.00"); }
            if (html.com !== null) { $(".comentario").html(html.com); } else { $(".comentario").html("Cliente no tiene comentarios"); }
        },
        error: function (error) {
            console.log('Disculpe, existió un problema');
            console.log(error);
