...
$query="SELECT idPedido FROM pedido ORDER BY idPedido ASC";
    ...
    if($rows>0){
        ...
        if($contador != 0){
            ...
            $query="SELECT me.nombreMenu, pp.estadoPedidoproducto FROM pedido pe join pedidoproducto pp on(pe.idPedido = pp.idPedido)
                        ...
                        where m.estadoMesa ='OCUPADA' AND pe.idPedido = '$idpedido'";
            ...
            if($contPedidoproducto != 0){
                $query="SELECT * FROM pedido pe join mesa m on(m.idMesa=pe.idMesa) where m.estadoMesa ='OCUPADA' AND pe.idPedido = '$idpedido'";
